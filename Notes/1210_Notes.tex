...
\begin{itemize}
  ...
  \item The AI talk\ldots \emph{Mastery} requires work!
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Practice: How many $g$ of \ch{Fe(NO3)2} are needed to make $250.0ml$ of a $0.125M$ solution?
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  Practice: Balance the chemical equation: \ch{(NH4)3PO4(aq) + Fe_2(CO3)3(aq) -> FePO4(s) + (NH4)2CO3(aq)}

  ~\hphantom{Practice:} \ch{2 (NH4)3PO4(aq) + Fe_2(CO3)3(aq) -> 2 FePO4(s) + 3 (NH4)2CO3(aq)}

  Practice: Balance the following chemical equation: \ch{C3H8(l) + O2(g) -> CO2(g) + H2O(g)}

  ~\hphantom{Practice:} \ch{C3H8(l) + 5 O2(g) -> 3 CO2(g) + 4 H2O(g)}
...
\begin{itemize}
  \item Electrolytes are covered in Chapter 11, but because they might appear on our final we will discuss them here too
  \item Electrolytes are chemical substances which yield ions when dissolved in water
    \begin{description}
      \item[Strong Electrolytes: ] Produce a \emph{stoichiometric} amount of ions. Stong acids/bases and soluble ionic compounds
      \item[Weak Electrolytes: ] Produce a \emph{sub-stoichiometric} amount of ions. Weak acids/bases
      \item[Non-Electrolytes: ] Do not produce any ions. Other soluble molecular compounds
    \end{description}
\end{itemize}
...
\begin{itemize}
  ...
  \item Hydrogenic Atoms
    \begin{itemize}
      \item Any atom or ion with only 1 electron will behave similarly, but with a different attractive force between the electron and the nucleus
      \item $Z$ is the nuclear charge
      \item For each of the equations above, just add a $Z^2$ in the numerator over $n^2$
    \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item Figure 8.1 shows the paramagnetic behavior of Oxygen, which cannot be explained by previous bonding theories
  \item Why should sharing electrons lead to a bond?
  \item Figure 8.2 shows how the energy changes with internuclear distance in \ch{H2}
  \item When the orbitals from both bonding partner overlap, these forces are optimized to form a bond
  \item Figure 8.4 shows a σ bond, where the overlap region is directly between the two bonding atoms
  \item Figure 8.5 shows a π bond, where the overlap regions are above and below the bond axis
  \item Every bond contains a σ bond, and multiple bonds add one or two π bonds
\end{itemize}
...
\begin{itemize}
  ...
  \item Figure 8.6 shows a model of water with $90^\circ$ bond angles (derpy water) predicted by atomic orbital overlap
  \item Hybridization is the linear combination (mixing) of atomic orbitals
    \begin{itemize}
      ...
      \item These hybridization types correspond to the electron geometries (Figure 8.7)
    \end{itemize}
  \item Different hybridization types
    \begin{description}
      \item[$sp$:] Linear geometry. Figures 8.8 and 8.9
      \item[$sp^2$:] Trigonal planar geometry. Figures 8.10-14
      \item[$sp^3$:] Tetrahedral geometry. Figures 8.15-17
      \item[$sp^3d$:] Trigonal bipyramidal geometry. Figures 8.18-19
      \item[$sp^3d^2$:] Octahedral geometry. Figure 8.20
    \end{description}
  \item Hybridization geometries are summarized in Figure 8.21
\end{itemize}

\paragraph*{Quiz 8.1 - Hybridization}
\paragraph*{Homework 8.1}
\begin{itemize}
  \item 9: Motivating hybridization
  \item 11: Limits on $sp^3d$ hybridization
  \item 13: Relating hybridization to geometry
  \item 15: Hybridization from Lewis Structures
\end{itemize}
...
\begin{itemize}
  ...
  \item For $sp$ and $sp^2$ hybridized atoms, unhybridized $p$ orbitals remain
  ...
  \item Double bonds
    \begin{itemize}
      \item Double bonds contain 1 π bond
      \item Figure 8.22 shows the energy levels of the orbitals in a double bond
      \item Figure 8.23 shows the orbitals of an ethene molecule
    \end{itemize}
  \item Triple bonds
    \begin{itemize}
      \item Triple bonds contain 2 π bonds
      \item Figure 8.24 shows the atomic and hybrid orbitals of an $sp$ atom
      \item Figure 8.25 shows the orbitals of an acetylene molecule (ethyne)
    \end{itemize}
  \item Multiple $p$ orbitals overlap in molecules which exhibit resonance (like benzene and nitrate) (Figure 8.26)
  \item Use BLMB Figures 9.23-9.28
  \item Valence bond theory and secondary structure
    \begin{itemize}
      ...
      \item $\pi$ bonds, however, would break if you twist them by $90^\circ$ because they lose their overlap
      \item $\pi$ bonds are constrained, and cannot rotate without a substantial energy input
    \end{itemize}
\end{itemize}

\paragraph*{Quiz 8.2 - Valence Bond Theory}
\paragraph*{Homework 8.2}
\begin{itemize}
  \item 23: Composition of a triple bond
  \item 25: Hybridization and multiple bonds
  \item 29: Hybridization and electron delocalization
\end{itemize}
...
\begin{itemize}
  \item In hybrid orbital theory, we combined atomic orbitals from the same atom to create new hybrid orbitals
	...
	\item Molecular orbitals can combine in two ways (Figures 8.28-29):
    \begin{itemize}
    ...
    \begin{itemize}
      \item Head-on combination creates two more $\sigma$ orbitals (Figure 8.30)
      \item Sideways combination creates orbitals with density along either side of the bond axis, so they are $\pi_{2p}$ and $\pi^*_{2p}$ MOs (Figure 8.31)
      ...
      \item The ordering of the $\pi_{2p}$ and $\sigma_{2p}$ orbitals is different for different elements (Figure 8.37)
    \end{itemize}
    ...
    \begin{itemize}
      ...
      \item Fill the MO energy levels from the bottom-up as usual (Figure 8.34)
      \item HOMO and LUMO orbitals are the most important for most chemical properties
    \end{itemize}
    ...
    \end{itemize}
	\item Figure 8.37 shows the configurations for all period 2 homonuclear diatomics (note the different orbital ordering)
  \item Figure 8.38 illustrates the change in orbital order due to s-p mixing (More mixing for Li-N)
  \item Figure 8.40 shows a complete molecular orbital energy level diagram for \ch{O2}
  \item We can also write configurations like for atoms, listing only each orbital and its occupancy (Table 8.3)
...
\paragraph*{Quiz 8.3 - Molecular Orbital Theory}
\paragraph*{Homework 8.3}
\begin{itemize}
  \item 41: Bond order of diatomic molecules
  \item 43: HOMO and ionization energy
  \item 47: Bond order of diatomic ions
  \item 49: MO diagrams and magnetic properties
\end{itemize}
...
\begin{itemize}
  ...
  \item Standard Temperature and Pressure (STP) is $0^{\circ}C$ and \emph{either} $1~atm$ \emph{or} $1~bar$, so until this dual usage is resolved it is unfortunately not a useful reference
\end{itemize}
...
\paragraph*{Quiz 9.1 - Gas Laws}
\paragraph*{Homework 9.1}
\begin{itemize}
  \item 5: Units of pressure
  \item 15: Measuring pressure with an open-end manometer
  \item 25: Using PV=nRT
  \item 27: Gas laws
  \item 33: PV=nRT and grams
  \item 45: Combined gas law
\end{itemize}
...
\begin{itemize}
  \item Dalton's Law of Partial Pressures
    ...
    \end{itemize}
  \item Molar Mass and Density in Gas Law Calculations
    \begin{itemize}
    ...
    \end{itemize}
  \item Gases in Chemical Reactions
    \begin{itemize}
...
\paragraph*{Quiz 9.2 - Calculations with Gases}
\paragraph*{Homework 9.2}
\begin{itemize}
  \item 49: Density of a gas sample
  \item 53: Molar mass from gas properties
  \item 57: Partial pressures
  \item 63: Partial pressure for a gas collected over water
  \item 65: Gas laws and reaction stoichiometry
\end{itemize}

\section{Effusion and Diffusion of Gases}
\begin{itemize}
  \item The equation for the \nth{5} postulate is: $KE_{avg}=\dfrac{3}{2}k_BT$
...
\paragraph*{Quiz 9.3 - Gas Kinetics}
\paragraph*{Homework 9.3}
\begin{itemize}
  \item 81: Comparing rates of effusion
  \item 93: Molar mass and speed distribution
  \item 99: Charts and non-ideal gas behavior
  \item 101: Conditions which deviate from ideality
  \item 103: Molecular volume of real gases
\end{itemize}
